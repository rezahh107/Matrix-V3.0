...
- **Reference SSoT:** پس از bootstrap، جدول‌های `schools` و `groupcodes` در LocalDatabase تنها منبع عملیاتی School/GroupCode هستند؛ Excel (SchoolReport/crosswalk) فقط برای import/به‌روزرسانی استفاده می‌شود و Database Tab سطح رسمی مشاهده/کنترل است.
...
- **قرارداد ستون‌ها:** ورودی Inspactor باید ستون «شامل گروه های آزمایشی» را داشته باشد؛ نبود این ستون ⇒ خطای Schema (P0) پیش از هر گونه parsing. ستون legacy «گروه آزمایشی» می‌تواند برای QA/دیباگ نگه داشته شود اما منبع join key محسوب نمی‌شود. این الزام فقط منبع گروه را تعریف می‌کند و ۶ کلید join همچنان همان مقادیر LAW را دارند.
- **اولویت منبع:** هیچ fallbackی به ستون legacy مجاز نیست. تنها «شامل گروه های آزمایشی» باید parse شود و گروه‌ها را تعیین کند؛ مقادیر legacy صرفاً می‌توانند در QA به‌عنوان دادهٔ غیرمرجع گزارش شوند.
...
- **Immutable allocations:** `allocations_df.student_id` از Core می‌آید و در لایهٔ Infra/Export تغییر نمی‌کند؛ نبود/تهی بودن آن باید fail-fast شود (LAW/EXPORT-SSOT-ID-01).
