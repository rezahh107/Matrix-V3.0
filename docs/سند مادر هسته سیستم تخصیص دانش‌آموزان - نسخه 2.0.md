...
## تعاریف به‌روز School vs Normal (Policy v1.0.3 / SSoT v1.0.2)
- **دانش‌آموز:**
  - alias عددی `<min(postal_valid_range)` (پیش‌فرض 1000) ⇒ مدرسه‌ای و باید با «کد مدرسه» منطبق شود (school_by_schoolcode).
  - alias در بازهٔ `postal_valid_range` ⇒ عادی و صرفاً با سطرهای شاخهٔ «عادی» و همان alias تطبیق می‌شود (normal_by_alias).
  - alias `>max(postal_valid_range)` یا غیرعددی ⇒ مدرسه‌ای و تطبیق روی شاخهٔ مدرسه‌ای با alias مبتنی بر mentor_id (school_by_mentorid).
- **MentorType (NORMAL/SCHOOL/DUAL):** از Inspactor و MentorSchoolBindingPolicy:
  - postal معتبر و بدون مدرسه ⇒ NORMAL.
  - مدرسه معتبر (>0) یا school_constraint اجباری ⇒ SCHOOL.
  - هر دو ⇒ DUAL (هر دو شاخه ساخته می‌شود).
  - اگر school_constraint فعال باشد و school_code نداشته باشیم، school_count=1 می‌شود تا شاخهٔ مدرسه‌ای حفظ شود.
- **Alias Rules:**
  - شاخهٔ عادی فقط با alias چهارنمری در بازهٔ policy ساخته می‌شود؛ خارج از بازه یعنی شاخهٔ عادی حذف می‌شود.
  - شاخهٔ مدرسه‌ای همیشه alias مبتنی بر mentor_id دارد؛ مقادیر <1000 در ولیدیشن قطعاً مدرسه‌ای هستند.
- **ماتریس 9354سطره:** افزوده شدن شاخهٔ مدرسه‌ای باعث افزایش سطرها نسبت به روایت 0918 (4212سطره) شده است؛ هنوز ۶ کلید join ثابت می‌مانند و سطرهای عادی/مدرسه‌ای در sort پایدار center→کدرشته→کد مدرسه→alias شماره‌گذاری می‌شوند.
...
- MentorProfile شامل `mentor_id`, ظرفیت‌های اعلام‌شده، مدیر بالادستی، متادیتای trace (مثلاً remaining_capacity و متریک‌های LEGACY مانند occupancy_ratio) و فیلد `mentor_status` است. Core هیچ نام یا شناسه‌ای را هاردکد نمی‌کند و صرفاً به همین ستون‌ها که از Policy/SSoT آمده‌اند اعتماد دارد.
- منطق فیلتر:
...
- این مرحله هیچ تغییری در ۶ کلید Join یا سیاست رتبه‌بندی LAW/TECH (`remaining_capacity` نزولی سپس `mentor_id` natural/stable) ایجاد نمی‌کند؛ روایت occupancy_ratio → allocations_new → mentor_id LEGACY محسوب می‌شود. mentor_status فقط تعیین می‌کند چه کسانی به ماتریس راه پیدا می‌کنند و دترمینیسم را پیش از شروع فیلترهای صلاحیت تضمین می‌کند.
...
- **ورودی‌ها:** allocation، allocation_summary (با ستون‌های ظرفیت باقی‌مانده؛ متریک‌های occupancy_ratio/allocations_new صرفاً در نسخه‌های قدیمی‌تر وجود داشته و LEGACY هستند).
- **Invariant:** برای هر mentor_id در روایت‌های قدیمی، `assigned_count ≤ remaining + allocations_new` و `occupancy_ratio = allocations_new / (remaining + allocations_new)` تعریف می‌شد؛ از LAW/TECH v3.0 به بعد صرفاً ظرفیت باقی‌مانده و گیت R0-CAPACITY-GATE-01 ملاک‌اند.
