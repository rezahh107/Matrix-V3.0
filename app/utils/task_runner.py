...
class TaskRunner(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            result = self.task_func(self.progress, self.check_cancel, *self.args, **self.kwargs)

            self.finished.emit(TaskResult(success=True, data=result))

        except Exception as e:
            error_result = TaskResult(success=False, error=str(e), traceback=traceback.format_exc())
