...
def validate_excel_file(path_str: str, file_desc: str = "فایل") -> Path:
    ...
    if not file_path.exists():
        raise ValidationError(f"{file_desc} یافت نشد\n" f"مسیر: {file_path}")
...
def validate_output_directory(path_str: str) -> Path:
    ...
    if not dir_path.exists():
        raise ValidationError(f"پوشه خروجی یافت نشد\n" f"مسیر: {dir_path}")
...
def validate_numeric_range(
    value: float, min_val: float, max_val: float, name: str = "مقدار"
) -> float:
    """
...
def validate_build_matrix_inputs(
    ...
    validator.add_check(
        lambda: results.update({"school": validate_excel_file(school_path, "گزارش schools")})
    )

    validator.add_check(
        lambda: results.update({"crosswalk": validate_excel_file(crosswalk_path, "فایل crosswalk")})
    )
