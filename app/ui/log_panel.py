...
class LogPanel(QFrame):
    ...
    def __init__(
        ...
        self._placeholder = QLabel(
            self._t("log.placeholder", "هنوز گزارشی ثبت نشده است."),
            stack_host,
        )
        ...
        buttons_col.setSpacing(theme.spacing_xs)
    ...
    def apply_theme(self, theme: Theme) -> None:
        """به‌روزرسانی palette/spacing؛ visual skin فقط در styles.qss است."""

        self._theme = theme
        root = self.layout()
        if isinstance(root, QHBoxLayout):
            root.setSpacing(theme.spacing_sm)
