...
### 3.3) Crosswalk.xlsx
- شیت **`پایه تحصیلی (گروه آزمایشی)`** با ستون‌های: **`گروه آزمایشی`**، **`کد گروه`**، **`مقطع تحصیلی`**، **`وضعیت تحصیلی مجاز`** ({0,1} یا {1})
- شیت اختیاری **`Synonyms`** با دو ستون (مثلاً from/to یا alias/target) برای سینونیم‌ها/باکت‌ها.
- **همسان‌سازی الگوها (v3.0):** تمام ورودی‌ها/خروجی‌ها باید دقیقاً از جدول وضعیت‌دار موجود در `educational_master.ssot.yaml` و Policy §4.2.5 پیروی کنند.
  - کدهای دوحالته = {1, 3, 5, 7, 8, 9, 11, 12, 14, 17, 18} → {0,1}; بقیه کدها فقط {1}.
  - تمایزهای مهم: کد 7 = هنر کنکوری (≠ هفتم) با وضعیت {0,1}، کد 33 = هفتم متوسطه اول {1}، کد 5 = دوازدهم انسانی (≠ پایه ۵) {0,1}، کد 41 = پنجم دبستان {1}.
