...
def test_database_manager_dialog_shows_counts_and_path(tmp_path: Path, qapp: QApplication) -> None:
    ...
    db.initialize()
    info = YearDatabaseInfo(
        "test-year", db.path, schema_version=_SCHEMA_VERSION, size_bytes=db.path.stat().st_size
    )
...
def test_database_manager_dialog_shows_schema_issue(tmp_path: Path, qapp: QApplication) -> None:
    ...
    with LocalDatabase(db_path).connect() as conn:
        conn.execute('CREATE TABLE students_cache("کد ملی" TEXT)')
        conn.execute(
            """
    ...
    issue_rows = [
        dialog._issues_table.item(r, 1).text() for r in range(dialog._issues_table.rowCount())
    ]
    assert any("student_id" in text for text in issue_rows)


def test_database_manager_buttons_trigger_actions(
    ...
    dialog = DatabaseManagerDialog(db=db, year_info=info)
    monkeypatch.setattr(
        "app.ui.database_manager_dialog.QMessageBox.information", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        "app.ui.database_manager_dialog.QMessageBox.critical", lambda *args, **kwargs: None
    )
    monkeypatch.setattr(
        "app.ui.database_manager_dialog.QMessageBox.question",
        lambda *args, **kwargs: QMessageBox.Yes,
    )

    calls = {"full": 0, "cache": 0}
    monkeypatch.setattr(
        db, "reset_full_database", lambda: calls.__setitem__("full", calls["full"] + 1) or db.path
    )
...
def test_full_reset_error_shows_message_box(
    tmp_path: Path, qapp: QApplication, monkeypatch
) -> None:
    if QApplication is None or not _QT_AVAILABLE:
        pytest.skip("Qt bindings not available")
