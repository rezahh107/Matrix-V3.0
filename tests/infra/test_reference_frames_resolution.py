...
def test_resolve_reference_frames_missing_tables_has_actionable_hint(monkeypatch, tmp_path):
    ...
    args = SimpleNamespace(schools=None, crosswalk=None)
...
def test_build_matrix_resolves_references_before_pool(monkeypatch, tmp_path):
    """در build-matrix ابتدا داده‌های مرجع باید بارگذاری شوند سپس استخر منتورها."""

    policy = load_policy()
    call_order: list[str] = []

    def fake_resolve_reference_frames(*, args, db):
        call_order.append("refs")
        return pd.DataFrame(), pd.DataFrame(), None, {}, {}

    def fake_resolve_mentor_pool_frame(*_args, **_kwargs):
        call_order.append("pool")
        return pd.DataFrame(), {}, {}

    def fake_build_matrix(*args, **kwargs):
    ...
    def fake_run_all_invariants(**kwargs):
        class _Report:
            passed = True
            violations: list = []
